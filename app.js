...
import { getFirestore, collection, onSnapshot, addDoc, deleteDoc, doc, query, orderBy, updateDoc, writeBatch, getDocs } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
Vue.createApp({
  data() {
    return {
      ...
      settings: JSON.parse(localStorage.getItem('vault_v16')) || { currency:"$", monthlyBudget: 2500, expenseCats:["Groceries","Rent","Dining","Gas","Fun"] },
      entry: { id: null, amount: null, category: 'Groceries', date: null }
    }
  },
  computed: {
    totalSpent() { return this.transactions.filter(t => t.category !== 'Paycheque').reduce((s, t) => s + (Number(t.amount) || 0), 0); },
    totalIncome() { return this.transactions.filter(t => t.category === 'Paycheque').reduce((s, t) => s + (Number(t.amount) || 0), 0); },
    ...
    categoryStats() {
        ...
        const cats = {};
        this.transactions.filter(t => t.category !== 'Paycheque').forEach(t => { cats[t.category] = (cats[t.category] || 0) + t.amount; });
  ...
  methods: {
    async login() { try { await signInWithEmailAndPassword(auth, this.loginEmail, this.loginPass); } catch (e) { alert("Login failed."); } },
    ...
    async wipeData() {
        if (confirm("⚠️ DANGER: This will delete ALL transactions for everyone. Are you sure?")) {
    ...
    exportCSV() {
      let csv = "Date,Category,Amount,By\n";
      this.transactions.forEach(t => csv += `${new Date(t.date).toLocaleDateString()},${t.category},${t.amount},${t.by}\n`);
      const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], {type:'text/csv'})); a.download='Vault.csv'; a.click();
      this.exportStatus = '✅ Exported!';
      setTimeout(() => { this.exportStatus = 'Export Data'; }, 2000);
    }
  },
  mounted() {
